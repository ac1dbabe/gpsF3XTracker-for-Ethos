--[[#############################################################################
SETUP: GPS F3X Tracker for Ethos v1.5
...
- v1.4: - logging of flight information
...
local lcoursedir = 0
local lcoursedif = 0
local lcoursedif_man = 0
local lcomptype = 1
local latitude = 0.0
local longitude = 0.0
local locations_choice = {{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}}
local choice_table = {{"f3f_training", 1}, {"f3f_competition", 2}, {"f3b_distance", 3}, {"f3b_speed", 4}, {"f3f_debug", 5}}
local gps_sensors = {{"SM Modellbau Logger3", 1}, {"FrSky GPS V2", 2}, {"RCGPS-F3x",3}, {"Any other GPS", 4}}

-- GLOBAL VARIABLES
-- global_gps_pos = ...                                     -- defined in the main main.lua
-- global_gps_gpsSats = ...                                 -- defined in the main main.lua
-- global_correctionFactor = 0                              -- defined in the main main.lua, flight position correction constant (0 = not used, 0.1 = suggested value, 1 = max value)
-- global_logging = false                                   -- defined in the main main.lua, switch enabling logging
...
global_comp_type = 'f3f_trai'                               -- default event type
global_comp_types = {
...
local function create()
  return {event_place=1, coursedir=0, comptype=1, coursedif=0, baseA_left=true, gps_type=1, gps_gpsSats = -1, lockGPSHomeSwitch=nil, coursedif_mgmt=nil}
end
-------------------------------------------------------------------------
-- wakeup function
-------------------------------------------------------------------------
local function wakeup(widget)
  ...
  if widget.event_place == 1 then                           -- for "Live" Position & Direction case
    ...
    elseif widget.coursedif_mgmt then                       -- lock GPS Home switch is activated & source for management of Course difference during flight configured
      local lcoursedif = math.floor(widget.coursedif_mgmt:value()/2)
      if lcoursedif_man ~= lcoursedif and math.abs(lcoursedif) <= 10 then       -- source value has changed and the value is in <-10, 10> meters limit
        lcoursedif_man = lcoursedif
        system.playNumber(lcoursedif_man, UNIT_METER, 1)
      end  
    end  
  end
...
--      print("Setup - update values from locations.locations table")
    ...
    lcoursedif_man = 0                                      -- manual course difference is valid only for "Live" event
  ...
  if global_course_dif ~= (widget.coursedif + lcoursedif_man) then              -- competition course length has changed
    print ("Setup - widget.coursedif changed", global_course_dif, widget.coursedif, lcoursedif_man)
    global_course_dif = widget.coursedif + lcoursedif_man
    screen.cleaned = false; global_has_changed = true
  end
...
--      print ("Setup - lon changed", global_home_pos.lon, lon)
  ...
  if global_gps_type ~= gps_sensors[widget.gps_type][1] then                    -- type of GPS sensor has changed
    ...
    screen.cleaned = false; global_gps_changed = true
  end
...
local function paint(widget)
  ...
  if lat == 0.0 and lon == 0.0 then
    ...
    if widget.lockGPSHomeSwitch:value() > 50 and widget.event_place == 1 then   -- for "Live" Position & Direction show when GPS Home coordinates are locked by lockGPSHomeSwitch
      screen.text(2, string.format("GPS Home lck: %10.7f, %10.7f", lat,lon))
    else
      screen.text(2, string.format("GPS Home: %10.7f, %10.7f", lat,lon))
    end  
  end
  ...
  screen.text(4, string.format("Course Difference: %2dm + %2dm", widget.coursedif, lcoursedif_man))
  screen.text(5, string.format("Competition Type: %s", choice_table[widget.comptype][1]))
  if widget.baseA_left then
    screen.text(6, "Course Base A: left")
  else
    screen.text(6, "Course Base A: right")
  end
  screen.text(7, string.format("GPS sensor: %s", gps_sensors[widget.gps_type][1]))
  if widget.gps_type == 1 or widget.gps_type == 3 then      -- display number of visible GPS satellites only for SM-Modelbau GPS-Logger3 and RCGPS-F3x
    screen.text(8, string.format("GPS satellites: %2d", global_gps_gpsSats))
  end  
end
-------------------------------------------------------------------------
-- Edit function
...
local function Edit(widget)
  ...
  form.clear()
  lname = locations[widget.event_place].name
  latitude = math.floor(lat * 10000000)                     -- 7 decimal places is supported in configuration below
  longitude = math.floor(lon * 10000000)
  ...
  LatFieldE = form.addNumberField(line, nil, -900000000, 900000000, function() return latitude end, function(value) latitude = value end)
  LatFieldE:decimals(7); LatFieldE:step(1); LatFieldE:suffix("°")
    
  line = form.addLine ("Longitude")                         -- Longitude
  LonFieldE = form.addNumberField(line, nil, -1800000000, 1800000000, function() return longitude end, function(value) longitude = value end)
  LonFieldE:decimals(7); LonFieldE:step(1); LonFieldE:suffix("°")
      ...
      press=function()
        if not (widget.event_place == 1 and len == 16) then -- No new site can be created, the list is full
          ...
          if widget.event_place == 1 then                   -- we create a new site from "Live Position & Direction" site
            ...
            file_target:write("    {name = ".."\""..lname.."\""..", lat = "..string.format("%10.7f",latitude/10000000)..", lon = "..string.format("%10.7f",longitude/10000000)..", dir = "..string.format("%5.1f",lcoursedir/10)..", dif = "..lcoursedif..", comp = "..lcomptype.."},\n")
          ...
          form.openDialog({buttons={{label="Saved", action=function() return true end}}, closeWhenClickOutside=true})
        else
          form.openDialog({buttons={{label="List of sites is full!", action=function() return true end}}, closeWhenClickOutside=true})
        end
      end
    })
end
...
local function configure(widget)
  ...
  line = form.addLine ("Course difference management")	     -- Source for management of Course difference during flight
	form.addSourceField (line, nil, function() return widget.coursedif_mgmt end, function(value) widget.coursedif_mgmt = value end)
end

local function read(widget)
  widget.event_place = storage.read("event_place")
--[[  widget.coursedir = storage.read("coursedir")    These items are stored in the Locations.lua
  ...
  widget.coursedif_mgmt = storage.read("coursedif_mgmt")
end
...
--[[  storage.write("coursedir", widget.coursedir)
  ...
  storage.write("coursedif_mgmt", widget.coursedif_mgmt)
end
-------------------------------------------------------------------------
-- init function
-------------------------------------------------------------------------
local function init()
    print("<<< INIT SETUP >>>")
    system.registerWidget({key="Gpstset", name="GPS F3X Tracker Setup v1.5", create=create, paint=paint, configure=configure, wakeup=wakeup, read=read, write=write})
