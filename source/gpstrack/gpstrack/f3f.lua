--[[#############################################################################
COMPETITION Library: F3F (traing and competition): GPS F3X Tracker for Ethos v1.5
...
- v1.4: - logging of flight information
        - deleted management of altitude in the comp.update() function - deletion of parameter height and variable comp.groundHeight
- v1.5:
################################################################################]]

local comp = {name='f3f.luac', baseAleft=true, mode='training', trainig=true, state=0, runtime=0, message='---'}
...
function comp.update()
  ...
  if comp.state == 30 then                                  -- 30: END
    ...
    comp.state = 0
    if global_logging then global_log_file:close() end      -- close the log file
    return
  end
end
